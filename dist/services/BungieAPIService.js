...
exports.BungieAPIService = void 0;
...
class BungieAPIService {
    ...
    async searchPlayer(displayName, membershipType) {
        try {
    ...
    async getPlayer(membershipType, membershipId) {
        try {
            console.log(`🔍 Recherche du profil: membershipType=${membershipType}, membershipId=${membershipId}`);
            const response = await this.apiClient.get(`/Destiny2/${membershipType}/Profile/${membershipId}/?components=100,200`);
    ...
    async getRecentActivities(membershipType, membershipId) {
        try {
            const response = await this.apiClient.get(`/Destiny2/${membershipType}/Account/${membershipId}/Character/0/Stats/Activities/?count=10&mode=0`);
            if (response.data.ErrorCode !== 1) {
                throw new Error(`Erreur API Bungie: ${response.data.Message}`);
            }
    ...
    async getCurrentActivity(membershipType, membershipId, characterId) {
        try {
            const response = await this.apiClient.get(`/Destiny2/${membershipType}/Profile/${membershipId}/Character/${characterId}/?components=204`);
    ...
    async getActivityDetails(activityHash) {
        try {
...
exports.BungieAPIService = BungieAPIService;
